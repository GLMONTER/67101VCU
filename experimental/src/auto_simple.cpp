...
void moveToTargetSimple(float y, float x, float ys, float xs, unsigned int power, unsigned startPower, float maxErrX, float decelEarly, byte decelPower, float dropEarly, tStopType stopType, tMttMode mode, bool velSafety)
{
	int velSafetyCounter = 0;
	if (LOGS) std::cout<<("Moving to %f %f from %f %f at %d", y, x, ys, xs, power);
	...
	if (LOGS) std::cout<<("Line length: %.2f", lineLength);
	...
	if (LOGS) std::cout<<("Line | Pid angle: %f | %f", radToDeg(lineAngle), radToDeg(pidAngle));
	...
	if (LOGS) std::cout<<("%f %f", currentPosVector.y, vel);
	...
	if (LOGS) std::cout<<("Moved to %f %f from %f %f | %f %f %f", y, x, ys, xs, gPosition.y, gPosition.x, radToDeg(gPosition.a));
}

void moveToTargetDisSimple(float a, float d, float ys, float xs, unsigned power, unsigned int startPower, float maxErrX, float decelEarly, unsigned int decelPower, float dropEarly, tStopType stopType, tMttMode mode, bool velSafety)
{
...
void turnToAngleNewAlg(float a, tTurnDir turnDir, float fullRatio, unsigned int coastPower, float stopOffsetDeg, bool mogo, bool harshStop, bool velSafety)
{
	if (LOGS) std::cout<<("Turning to %f", radToDeg(a));
	...
	case cw:
		...
		if (LOGS) std::cout<<("Turn done: %d",  gPosition.a);
		if (harshStop)
		{
			setDrive(-20, 20);
			sleep(150);
			if (LOGS) std::cout<<("Break done: %d",  gPosition.a);
		}
		setDrive(0, 0);
		break;
	case ccw:
	...
	if (LOGS) std::cout<<("Turned to %f | %f %f %f", radToDeg(a), gPosition.y, gPosition.x, radToDeg(gPosition.a));
}
...
{
	if (LOGS) std::cout<<("Turning to %f %f", y, x);
	...
	case cw:
		...
		if (LOGS) std::cout<<("%f %f", radToDeg(target), radToDeg(endFull));
		setDrive(127, -127);
	...
	if (LOGS) std::cout<<("Turned to %f %f | %f %f %f", y, x, gPosition.y, gPosition.x, radToDeg(gPosition.a));
}
...
void sweepTurnToTarget(float y, float x, float a, float r, tTurnDir turnDir, byte power, bool slow, bool velSafety)
{
	...
	case cw:
		...
		if (LOGS) std::cout<<("%d Sweep to %f around %f %f", nPgmTime, radToDeg(a), yOrigin, xOrigin);
	...
	setDrive(0, 0);
	if (LOGS) std::cout<<("%d Done sweep turn", nPgmTime);
}
